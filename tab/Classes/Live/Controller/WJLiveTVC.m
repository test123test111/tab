...
/** 横幅view */
@property (nonatomic, strong) WJBannerV *bannnerV;

//由欢迎引导窗口GET到的数据传过来
@property (nonatomic, strong) NSDictionary *rstDict;
...
/** tableView显示数组 */
@property (nonatomic, strong) NSArray *showMAry;
...
- (instancetype)init
{
    self = [super init];
    if (self) {
        [[NSNotificationCenter defaultCenter] addObserver:self selector:@selector(ncLoadLiveAllData:) name:@"ncLoadLiveAllData" object:nil];
    }
    return self;
}
...
- (void)ncLoadLiveAllData:(NSNotification *)notification
{
    if(notification.object) {
        self.rstDict = notification.object;
    }
}

- (void)dealloc
{
    [[NSNotificationCenter defaultCenter] removeObserver:self name:@"ncLoadLiveAllData" object:nil];
}
...
- (void)leftBtnClick:(UIButton *)btn
{
    ...
    self.showMAry = self.liveingMAry;
    [self.tableView reloadData];
}
...
- (void)rightBtnClick:(UIButton *)btn
{
    ...
    self.showMAry = self.noticeLiveMAry;
    [self.tableView reloadData];
}
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    cell.ShowM = self.showMAry[indexPath.row];
